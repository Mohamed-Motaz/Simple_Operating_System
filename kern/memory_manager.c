...
{
	//TODO:DONE [PROJECT 2022 -[5] Kernel Dynamic Allocation] create_page_table()
	uint32 *pageTableArr = (uint32*) kmalloc(PAGE_SIZE);

	uint32 pageTablePa = kheap_physical_address(
			(uint32)pageTableArr);

	if (pageTableArr == NULL)
		return NULL;
